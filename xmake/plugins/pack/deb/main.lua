...
function _get_qt_dependencies(package)
    ...
    if not qt then
        print("Warning: Qt SDK not found, using generic Qt dependencies")
    ...
    print("Detected Qt version:", qt_version, "major:", qt_major)
    ...
    print("Generated Qt runtime dependencies:", table.concat(qt_deps, ", "))
    print("Generated Qt build dependencies:", table.concat(qt_build_deps, ", "))
...
function _remove_rpath_from_binaries(package)
    ...
    if not chrpath then
        print("chrpath not found, cannot remove RPATH automatically")
        return {}
    end
            ...
            if os.isfile(binary) then
                ...
                if file_output and (file_output:find("ELF.*executable") or file_output:find("ELF.*shared object")) then
                    ...
                    if chrpath_output and (chrpath_output:find("RPATH=") or chrpath_output:find("RUNPATH=")) then
                        ...
                        print("Will remove RPATH from:", rel_path)
                    end
                end
            end
        end
...
function _get_specvars(package)
    ...
    if is_qt then
        print("Detected Qt project - will include Qt-specific dependencies")
    end
