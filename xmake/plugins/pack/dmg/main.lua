...
function _get_create_dmg()
    ...
    if not create_dmg then
        print("Using basic hdiutil for DMG creation. Install create-dmg for better appearance: brew install create-dmg")
    end
    return create_dmg
end
    ...
    if iconfile and os.isfile(iconfile) then
        ...
        if not iconfile:endswith(".icns") then
            ...
            if sips and iconutil then
                ...
                print("Warning: Icon conversion tools not found, copying original file")
...
-- collect dependencies using otool
function _collect_deps_manually(package, appbundle_dir)
    ...
    if copied_count > 0 then
        print("Collected " .. copied_count .. " dependencies")
    end
    return true
end
...
function _create_basic_dmg(hdiutil, package, dmg_staging_dir, dmg_file, config, appbundle_name)
    ...
    if not os.isfile(temp_dmg) then
        print("Error: Temporary DMG file was not created")
        return false
    end
    
    -- 挂载DMG
    local mount_args = {"attach", "-readwrite", "-noverify", "-noautoopen", temp_dmg}
    local mount_output = os.iorunv(hdiutil.program, mount_args)
    ...
    if not os.isdir(mount_point) then
        print("Error: Mount point directory does not exist")
...
function _pack_dmg_main(hdiutil, create_dmg, codesign, package)
    ...
    for _, cmd in ipairs(installcmds) do
        ...
        if not ok then
            print("Warning: Install command failed: " .. cmd)
        end
    end
    ...
    if success then
        ...
        if dmg_info then
            ...
            if size then
                ...
                print(string.format("DMG created: %s (%d MB)", dmg_file, size_mb))
            else
                print("DMG created: " .. dmg_file)
            end
        end
    end
...
function main(package)
    ...
    if success then
        ...
        if os.isfile(dmg_file) then
            ...
            if file_info then
                print("Final file: " .. file_info:match("[^\r\n]+"))
            end
        end
    else
