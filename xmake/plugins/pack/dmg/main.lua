...
function _get_create_dmg()
    ...
    if not create_dmg then
        print("Using basic hdiutil for DMG creation (create-dmg not found).")
    end
    return create_dmg
end
        ...
        if not iconfile:endswith(".icns") then
            ...
            if sips then
                ...
                for _, size in ipairs(sizes) do
                    local output_path = path.join(temp_iconset, string.format("icon_%dx%d.png", size, size))
                    os.runv(sips.program, {"-z", tostring(size), tostring(size), iconfile, "--out", output_path})
                    if size <= 512 then
                        local output_path_2x = path.join(temp_iconset, string.format("icon_%dx%d@2x.png", size, size))
    ...
    for line in otool_output:gmatch("[^\r\n]+") do
        ...
        if lib_path and not lib_path:startswith("/usr/lib/") and not lib_path:startswith("/System/") and os.isfile(lib_path) then
            ...
            if not os.isfile(dst_path) then
                if os.runv("cp", {lib_path, dst_path}) and install_name_tool then
...
function _create_basic_dmg(hdiutil, package, dmg_staging_dir, dmg_file, config, appbundle_name)
    ...
    local required_size = 100
    if du_output then
        local size_match = du_output:match("^(%d+)")
        if size_match then
            required_size = math.ceil(tonumber(size_match) * 1.5)
        end
    end
    ...
    local create_args = {"create", "-size", required_size .. "m", "-fs", "HFS+", "-volname", config.title, temp_dmg}
    local ok = os.runv(hdiutil.program, create_args)
    if not ok then
        create_args[3] = "50m"
        ok = os.runv(hdiutil.program, create_args)
    ...
    if not mount_point or not os.isdir(mount_point) then
        os.tryrm(temp_dmg)
        return false
    end
    ...
    if not os.runv("cp", {"-R", app_source, app_dest}) then
    ...
    local convert_args = {"convert", temp_dmg, "-format", config.format, "-imagekey", "zlib-level=9", "-o", dmg_file}
    local convert_ok = os.runv(hdiutil.program, convert_args)
...
function _create_enhanced_dmg(hdiutil, create_dmg, package, dmg_staging_dir, dmg_file)
    ...
    if create_dmg then
        local args = {
            "--volname", config.title, "--window-pos", "200", "120", "--window-size", "600", "400",
            "--icon-size", "100", "--icon", appbundle_name, "175", "220",
            "--hide-extension", appbundle_name, "--app-drop-link", "425", "220",
            dmg_file, dmg_staging_dir
        }
        return os.runv(create_dmg.program, args)
    else
...
function _pack_dmg_main(hdiutil, create_dmg, codesign, package)
    ...
    if codesign then
        os.runv(codesign.program, {"-s", package:get("codesign.identity"), appbundle_dir})
    end
    ...
    if success and os.isfile(dmg_file) then
        _verify_dmg(hdiutil, dmg_file)
    end
    
    os.tryrm(dmg_staging_dir)
    
    return success, dmg_file
end
...
function main(package)
    ...
    if dmg_file ~= original_dmg_file then
        print(string.format("Corrected DMG filename: '%s' -> '%s'", original_dmg_file, dmg_file))
    end
    ...
    local success, final_dmg_file = _pack_dmg_main(hdiutil, create_dmg, codesign, package)
    
    if success then
        print(string.format("DMG packaging completed successfully: %s", final_dmg_file))
    else
